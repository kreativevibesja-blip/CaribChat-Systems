    ...
    sock.ev.on('messages.upsert', async (m) => {
      try {
        ...
        for (const msg of msgs) {
          ...
          ensureConversation(from).catch(()=>{});
          io.emit('message:new', { peer: from });
...
io.on('connection', (socket)=>{
  if (socket.data.userId){
    onlineAgents.add(socket.data.userId);
    io.emit('presence', { online: Array.from(onlineAgents) });
  }
...
// Simple signup alias (same behavior as login creates user if missing)
...
// Forgot password placeholder (log-only)
...
app.get('/api/conversations', authMiddleware, async (req, res)=>{
  try{
    ...
    const rows = await listConversations(200, filter, req.user?.id||null);
    res.json(rows);
  } catch(e){ res.status(500).json({ error:'list conversations failed' }); }
});
app.post('/api/conversations/assign', authMiddleware, async (req, res)=>{
  try{
    const { peer, userId } = req.body||{};
    if (!peer) return res.status(400).json({ error:'peer required' });
    const assigned = await assignConversation(peer, userId || req.user.id);
    io.emit('conversation:assigned', { peer, userId: assigned.assigned_user_id });
    res.json(assigned);
  } catch(e){ res.status(500).json({ error:'assign failed' }); }
});
app.post('/api/conversations/unassign', authMiddleware, async (req, res)=>{
  try{
    const { peer } = req.body||{};
    if (!peer) return res.status(400).json({ error:'peer required' });
    const un = await unassignConversation(peer);
    io.emit('conversation:assigned', { peer, userId: null });
    res.json(un);
  } catch(e){ res.status(500).json({ error:'unassign failed' }); }
});
