...
function ensureConversation(peer){
  const row = db.prepare(`SELECT * FROM conversations WHERE peer = ?`).get(peer);
  if (row) return row;
  db.prepare(`INSERT INTO conversations (peer) VALUES (?)`).run(peer);
  return db.prepare(`SELECT * FROM conversations WHERE peer = ?`).get(peer);
}
function listConversations(limit=200, filter='all', userId=null){
  // Distinct peers from messages with last time
  const rows = db.prepare(`
    SELECT c.peer,
           cv.assigned_user_id,
           MAX(m.created_at) as last_time
    FROM (
      SELECT from_number as peer FROM messages
      UNION
      SELECT to_number as peer FROM messages
    ) c
    LEFT JOIN conversations cv ON cv.peer = c.peer
    LEFT JOIN messages m ON (m.from_number=c.peer OR m.to_number=c.peer)
    GROUP BY c.peer
...
function assignConversation(peer, userId){
  ensureConversation(peer);
  db.prepare(`UPDATE conversations SET assigned_user_id=?, assigned_at=datetime('now') WHERE peer=?`).run(userId, peer);
  return db.prepare(`SELECT * FROM conversations WHERE peer=?`).get(peer);
}
function unassignConversation(peer){
  ensureConversation(peer);
  db.prepare(`UPDATE conversations SET assigned_user_id=NULL, assigned_at=NULL WHERE peer=?`).run(peer);
  return db.prepare(`SELECT * FROM conversations WHERE peer=?`).get(peer);
}
